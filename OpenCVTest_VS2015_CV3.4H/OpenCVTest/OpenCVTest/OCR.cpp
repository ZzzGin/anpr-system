...
OCR::OCR(string trainFile)
{
	...
	// 读取OCR.xml文件
	FileStorage fs;
	fs.open("D:\\GraduationProject\\OpenCVTest_VS2015_CV3.4H\\OpenCVTest\\OpenCVTest\\OCR.xml", FileStorage::READ);
	Mat TrainingData;
	Mat Classes;
...
bool OCR::verifySizes(Mat r) {
	...
	// 若一块区域的比率超过标准比率的80%，则认为该区域为黑色块，而不是一个字符
	if (DEBUG)
...
vector<CharSegment> OCR::segment(Plate plate) {
	...
	Mat thresholdImage;
	threshold(input, thresholdImage, 200, 255, CV_THRESH_BINARY);
	if (DEBUG)
...
void OCR::train(Mat TrainData, Mat classes, int nlayers) {
	...
	trained = AnN->train(TrainData, ROW_SAMPLE, trainClasses);
