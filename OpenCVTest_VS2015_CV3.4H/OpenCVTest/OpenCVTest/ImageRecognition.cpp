...
bool ImageRecognition::verifySizes(RotatedRect ROI) {
	// To set the param to verify the RectSize
	float error = 0.4;
	//Chinese car plate size: 440x140 aspect 3.1428
	float aspect = 3.1428;
	//Set a min and max area. All other patches are discarded
	int min = 15 * aspect * 15;	// 100 pixel
	int max = 125 * aspect * 125;	// 400 pixel
...
vector<Plate> ImageRecognition::segment(Mat input)
{
	...
	blur(grayImage, grayImage, Size(5, 5));		// 去除噪声
	if (showSteps)
		imshow("cvt&blur", grayImage);
	...
	for (int i = 0; i< rects.size(); i++) {
		...
		for (int j = 0; j < NumSeeds; j++) {
			Point seed;
			seed.x = rects[i].center.x + j*(minSize / 20) - minSize / 2;
			seed.y = rects[i].center.y + j*(minSize / 20) - minSize / 2;
			if (seed.x >= input.cols || seed.y >= input.rows || seed.x < 0 || seed.y < 0)	//be careful of the boundary
				continue;
			circle(result, seed, 1, Scalar(0, 255, 255), -1);
			// 运用填充算法，参数已设置
			area = floodFill(input,
				mask,
				seed,
				Scalar(255, 0, 0),
				&ccomp,
			...
			if (area > 55000) { // 去除错误的区域防止出现过大的区域影响运行时间
				break;
			}
		}
		if (area > 55000) { // 去除错误的区域防止出现过大的区域影响运行时间
		...
		if (verifySizes(minRect)) {
			...
			Mat resultResized;
			resultResized.create(35, 110, CV_8UC3);
