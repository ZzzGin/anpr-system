...
void usmart_scan(void)
{
	...
	{					   
		...
		if (USART_RX_BUF[0]=='0'&&USART_RX_BUF[1]=='x'&&USART_RX_BUF[2]=='0'){ //链接成功标记
				printf("Connected!\r\n");
		}
		else if (USART_RX_BUF[0]=='0'&&USART_RX_BUF[1]=='x'&&USART_RX_BUF[2]=='1'){	//放行标记
			int i=0;
			while (i<=800){	
				key233=KEY_Scan(0);
				if(key233){
				...
				if (interact){
					LCD_Fill(1,1,200,200,RED);
				}else{LCD_Fill(1,1,200,200,GREEN);}
				i++;
			}
			printf("GOGO!\r\n");
		}
		else if (USART_RX_BUF[0]=='0'&&USART_RX_BUF[1]=='x'&&USART_RX_BUF[2]=='2'){	//拒绝标记
			int i=0;
			while (i<=1600){
				key233=KEY_Scan(0);
				if(key233){
				...
				if (interact){
					LCD_Fill(1,1,200,200,GREEN);
				}else{LCD_Fill(1,1,200,200,RED);}			
				i++;
			}
			printf("NONO!\r\n");
		}
		else if (USART_RX_BUF[0]=='0'&&USART_RX_BUF[1]=='x'&&USART_RX_BUF[2]=='3'){	//怀疑标记
			int i=0;
			while (i<=800){	
				LCD_Fill(1,1,200,200,YELLOW);
				i++;
			}
			printf("ISIS?\r\n");
		}
